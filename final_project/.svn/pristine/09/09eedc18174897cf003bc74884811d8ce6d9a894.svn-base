...
function updateEmp(p){
	data = {};
	data["secCd"] = $('#dt_secCd').text();
	data["empCd"] = $('#dt_empCd').text();
	...
	$.ajax({
		...
		success: function(res){
			...
			switch(res.deptNm){
			case "진료부" : 
				$("#upd_select").val("optionMed").trigger('change');
				$("#up_secCd").val(res.secCd).trigger('change');
				$("#up_position").val(res.position).trigger('change');
				break;
				
			case "간호부" : 
				$("#upd_select").val("optionNur").trigger('change');
				$("#up_secCd").val(res.secCd).trigger('change');
				$("#up_position").val(res.position).trigger('change');
				break;
				
			case "의료기사부" : 
				$("#upd_select").val("optionEng").trigger('change');
				$("#up_secCd").val(res.secCd).trigger('change');
				$("#up_position").val(res.position).trigger('change');
				break;
				
			case "행정부" : 
				$("#upd_select").val("optionAdm").trigger('change');
				$("#up_secCd").val(res.secCd).trigger('change');
				$("#up_position").val(res.position).trigger('change');
				break;
			}
			
// 			$("#upd_select").val(res.deptNm);
// 			$("#up_secCd").val(res.secCd);
			$("#up_position").val(res.position);
...
// 	$('html').loading('start');
	var empCd = $('#empCd').val();
	var secCd = $('#secCd').val();
	var major = $('#secCd option:selected').text();
    ...
    data["hireDate"]=hireDate.replaceAll('-','') + '090000';
    data["empPw"]=empPw;
...
<h2>직원목록</h2>
<h1 id="secName" hidden="true">${secNm}</h1>
...
<!-- 					<button class="dropdown-item detailPartBtn" type="button" value="SEC00001" onclick="selectDept(this)">내과</button> -->
					<a class="dropdown-item detailPartBtn" href="/emp/home?secCd=SEC00001&keyword=SEC00001&secNm=내과">내과</a>
					<a class="dropdown-item detailPartBtn" href="/emp/home?secCd=SEC00002&keyword=SEC00002&secNm=외과">외과</a>
					<a class="dropdown-item detailPartBtn" href="/emp/home?secCd=SEC00003&keyword=SEC00003&secNm=정형외과">정형외과</a>
					<a class="dropdown-item detailPartBtn" href="/emp/home?secCd=SEC00004&keyword=SEC00004&secNm=마취과">마취과</a>
				</div>
					
				<button class="dropdown-item mainPartBtn" type="button">간호부</button>
				<div class="btn-group show">
				<button class="btn btn-Info dropdown-toggle partBtn" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" hidden="hidden">진료부</button>
					<a class="dropdown-item detailPartBtn" href="/emp/home?secCd=SEC00005&keyword=SEC00005&secNm=외래간호과">외래간호과</a>
					<a class="dropdown-item detailPartBtn" href="/emp/home?secCd=SEC00006&keyword=SEC00006&secNm=특수간호과">특수간호과</a>
					<a class="dropdown-item detailPartBtn" href="/emp/home?secCd=SEC00007&keyword=SEC00007&secNm=병동간호과">병동간호과</a>
				</div>
					
				<button class="dropdown-item mainPartBtn" type="button">행정부</button>
				<div class="btn-group show">
				<button class="btn btn-Info dropdown-toggle partBtn" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" hidden="hidden">진료부</button>
					<a class="dropdown-item detailPartBtn" href="/emp/home?secCd=SEC00009&keyword=SEC00009&secNm=원무과">원무과</a>
					<a class="dropdown-item detailPartBtn" href="/emp/home?secCd=SEC00010&keyword=SEC00010&secNm=약제과">약제과</a>
				</div>		
				
				<button class="dropdown-item mainPartBtn" type="button">의료기사부</button>
				<div class="btn-group show">
				<button class="btn btn-Info dropdown-toggle partBtn" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" hidden="hidden">진료부</button>
					<a class="dropdown-item detailPartBtn" href="/emp/home?secCd=SEC00008&keyword=SEC00008&secNm=방사선과">방사선과</a>
				</div>
				
				<button class="dropdown-item mainPartBtn" type="button">전체</button>
				<div class="btn-group show">
				<button class="btn btn-Info dropdown-toggle partBtn" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" hidden="hidden">진료부</button>
					<a class="dropdown-item detailPartBtn" href="/emp/home?secNm=전체">전체</a>
				</div>
...
<!--------------------------- 부서별 직원 목록 출력 시작--------------------------->		
		<c:forEach var="list" items="${list}" varStatus="i">
			<c:if test="${list.stateCd ne '최초로그인'}">
			...
			<td><button class="btn empDetailBtn" class="btn btn-outline-Secondary" type="button" onclick="empDetail(this)" id="${list.empCd}">${list.empNm}</button>
			</td>
