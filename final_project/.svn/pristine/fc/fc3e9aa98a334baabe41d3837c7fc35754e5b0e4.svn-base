...
// 	console.log('${loginUser.deptNm}')
	...
	$.ajax({
		...
		success: function(res) {
			...
			$.each(res, function(i, v) {
				option += '<option value="'+v.pntCd+'">'+v.pntCd+' '+v.pntNm+' '+v.pntPrno.split('-')[0]+'</option>';
			})
		 		...
		 		success: function (res) {
		 			...
		 			for(var i=1; i<hourval.length; i++){
		 				...
		 				for(var j=0; j<res.length; j++){
		 					if(hourval[i] == res[j].rsvTime || hourval[i] <= currenttime){
...
<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog"
	...
	내원 사유 : 
	<br>
					<textarea id="symptoms" name="symptoms" cols="45" rows="4"
